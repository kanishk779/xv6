...
inline void sdid_it_rund(int x)
{
  rseed = x;
}
...
getpinfo(struct proc_stat *p)
{
  ...
  p->current_queue = myproc()->q_number;
  int i;
...
scheduler(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    if(p->state == RUNNABLE) 
      runnable_processes[p->q_number]++;
    if(ticks - p->exit_time > queue_aging_limit[p->q_number])
    {
      if(p->q_number > 0)
      {
        process_count[p->q_number--]--;
        process_count[p->q_number]++;
      }
    }
  }
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if(p->q_number == selectedQueue && p->state == RUNNABLE)
    {
      p->q_position--;
      if(p->q_position < 0)
      {
...
yield(void)
{
  ...
  if(myproc()->localrtime > time_quantum[myproc()->q_number])
  {
    ...
    if(myproc()->q_number < 4)
    {
      process_count[myproc()->q_number++]--;
      process_count[myproc()->q_number]++;
      myproc()->q_position = process_count[myproc()->q_number]++;
    }
    else
    {
      myproc()->q_position = process_count[4];
    }
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d %s %s q_number[%d] q_position[%d]", p->pid, state, p->name, p->q_number, p->q_position);
    if(p->state == SLEEPING){
...
int updateq_position(int q_number)
{
  acquire(&ptable.lock);
  struct proc *p;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    // increase the position of each process in the respective queues
    if(p->q_number == myproc()->q_number)
    {
      p->q_position++;
    }
  }
